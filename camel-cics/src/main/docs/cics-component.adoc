...
This component allows you to interact with https://www.ibm.com/products/cics-transaction-server[CICS^®^]
...
Where `interfaceType` is the CICS set of the external API that this component will invoke.
At the moment, only ECI (External Call Interface) is supported.
This component can communicate with CICS server using two kinds of `dataExchangeType`
...
using COMMAREA. The COMMAREA size has to be defined in `CICS_COMM_AREA_SIZE`
...
a `java.util.Map<String,Object>` where the keys are the container names and the values are the input data.
